...
import 'package:Sicontigo_Visita_Domiciliaria/viewmodels/UI/menu_deOpcionesVisitaDos.dart';
...
class CameraHome extends StatefulWidget {
  ...
  /// Default Constructor
  CameraHome(this.formDataModel, this.index, {super.key});
...
class _CameraHomeState extends State<CameraHome>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: HelpersViewCabecera.formItemsDesign("Capturar foto", context),
      ),
      body: Column(
        children: <Widget>[
          Expanded(
    ...
    takePicture().then((XFile? file) async {
      if (mounted) {
        ...
        if (file != null) {
          ...
          if(widget.index == 0) {
            widget.formDataModel!.fotoUno = base64Encode(await file.readAsBytes());
            widget.formDataModel!.fechaTabletFotoUno = formatDate(
                "dd/MM/yyyy hh:mm:ss", DateTime.now());
          } else if(widget.index == 1) {
            widget.formDataModel!.fotoDos = base64Encode(await file.readAsBytes());
            widget.formDataModel!.fechaTabletFotoDos = formatDate(
                "dd/MM/yyyy hh:mm:ss", DateTime.now());
          } else if(widget.index == 2) {
            widget.formDataModel!.fotoTres = base64Encode(await file.readAsBytes());
            widget.formDataModel!.fechaTabletFotoTres = formatDate(
                "dd/MM/yyyy hh:mm:ss", DateTime.now());
          } else if(widget.index == 3) {
            widget.formDataModel!.fotoCuatro = base64Encode(await file.readAsBytes());
            widget.formDataModel!.fechaTabletFotoCuatro = formatDate(
                "dd/MM/yyyy hh:mm:ss", DateTime.now());
          }
          ...
          switch (codigoForm) {
            ...
            case Resources.valor_primeraVisita:
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => MenudeOpcionesVisitaUno(widget.formDataModel)),
              );
            ...
            case Resources.valor_segundaVisita:
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => MenudeOpcionesVisitaDos(widget.formDataModel)),
              );
            case Resources.valor_terceraVisita:
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => MenudeOpcionesVisitaTres(widget.formDataModel)),
              );
            ...
            case Resources.valor_percepciones:
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => MenudeOpcionesPercepcion(widget.formDataModel)),
              );
